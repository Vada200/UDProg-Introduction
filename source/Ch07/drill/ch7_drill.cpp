...
constexpr char print = ';'; //=-nem lehet mert duplicate case
...
Token Token_stream::get()
{
	...
	switch(ch) {
		...
		default: //nem szám vagy operátor -> egyéb
		{
			if(isalpha(ch)){
				string s;
				s += ch;
...
void Token_stream::ignore(char c) //implement
{
	...
	char ch = 0;
	while (cin>>ch) //karaktereket dobjuk el és nem tokeneket
		if (ch == c) return;
}
...
double calc_pow() {
	...
	if ( kitevo < 0) error("pow: negative value");
...
void clean_up_mess(){ //rossz tokenek eltávolítása
	ts.ignore(print);
